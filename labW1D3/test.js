...
function mochaTests() {
    ...
    describe("Analyser", function () {
        it("analysis properties", function () {
			...
			person.f = analyzer;	// name too short //add unbounded this
			person.x = 0;			// name too short
			let analysis = person.f();
            assert.equal(analysis.numProperties, 6);
			assert.equal(analysis.cntShortName, 2);
			assert.equal(analysis.cntReference, 1);	// the grades array is a reference. count reference , if the type of the varible is object then it is reference
        });
	});	
